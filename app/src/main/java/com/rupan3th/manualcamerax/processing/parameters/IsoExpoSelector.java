...
public class IsoExpoSelector {
    ...
    public static ExpoPair GenerateExpoPair(int step, CaptureController captureController) {
        ...
        double compensation = Math.pow(2.0,ManualCameraX.getSettings().exposureCompensation);
        pair.normalizeiso100();
        pair.ExpoCompensateLower(1.0/compensation);
        if (ManualCameraX.getSettings().selectedMode == CameraMode.NIGHT)
        {
            mpy1 = 7000.0;
            //if(step%3 == 2) mpy = 1.1;
            //mpy = mpy*1.5;
        ...
        if (pair.exposure < ExposureIndex.sec / 8 && pair.normalizedIso() > 1500.0/mpy1) {
            if (step != baseFrame || !ManualCameraX.getSettings().eisPhoto) pair.ReduceIso();
        }
        if (pair.exposure < ExposureIndex.sec / 8 && pair.normalizedIso() > 1500.0/mpy1) {
            if (step != baseFrame || !ManualCameraX.getSettings().eisPhoto) pair.ReduceIso(1.25);
        }
        ...
        if (pair.exposure < ExposureIndex.sec / 90 && ManualCameraX.getSettings().eisPhoto) {
            //HDR = true;
        }
        if (step == baseFrame) {
            if (pair.normalizedIso() <= 120.0/mpy1 && pair.exposure > ExposureIndex.sec / 70.0/mpy1 && ManualCameraX.getSettings().eisPhoto) {
                pair.ReduceExpo();
            }
            if (pair.normalizedIso() <= 245.0/mpy1 && pair.exposure > ExposureIndex.sec / 50.0/mpy1 && ManualCameraX.getSettings().eisPhoto) {
                pair.ReduceExpo();
            }
            if (pair.exposure < ExposureIndex.sec * 3.00 && pair.exposure > ExposureIndex.sec / 3 && pair.normalizedIso() < 3200.0/mpy1 && ManualCameraX.getSettings().eisPhoto) {
                pair.FixedExpo(1.0 / 8);
                if (pair.normalizeCheck())
                    ManualCameraX.showToast("Wrong parameters: iso:" + pair.iso + " exp:" + pair.exposure);
            }
        }
        if(step != -1) {
