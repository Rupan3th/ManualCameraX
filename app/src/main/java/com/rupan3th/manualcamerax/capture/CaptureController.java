...
import com.rupan3th.manualcamerax.api.CameraEventsListener;
import com.rupan3th.manualcamerax.api.CameraManager2;
import com.rupan3th.manualcamerax.api.CameraMode;
import com.rupan3th.manualcamerax.api.CameraReflectionApi;
...
import com.rupan3th.manualcamerax.manual.ParamController;
import com.rupan3th.manualcamerax.processing.ImageSaver;
import com.rupan3th.manualcamerax.processing.parameters.ExposureIndex;
...
import com.rupan3th.manualcamerax.ui.camera.viewmodel.TimerFrameCountViewModel;
import com.rupan3th.manualcamerax.ui.camera.views.viewfinder.AutoFitPreviewView;
import com.rupan3th.manualcamerax.ui.camera.views.viewfinder.GLPreview;
import com.rupan3th.manualcamerax.util.log.Logger;
...
//            mImageSaver.processingHandler.sendMessage(msg);
        ...
        public void onCaptureStarted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, long timestamp, long frameNumber) {
            ...
            if (frameNumber % 20 == 19) {
                ...
                if (ExposureIndex.index() + 0.9 < 8.0) {
                    if (is30Fps && ManualCameraX.getSettings().fpsPreview && !mCameraDevice.getId().equals("1")) {
    ...
    public final TextureView.SurfaceTextureListener mSurfaceTextureListener
        ...
        public void onSurfaceTextureAvailable(@NonNull SurfaceTexture texture, int width, int height) {
            Size optimal = getPreviewOutputSize(mTextureView.getDisplay(),
                    mCameraCharacteristicsMap.get(ManualCameraX.getSettings().mCameraID),
    ...
    public CaptureController(Activity activity, ExecutorService processExecutor, CameraEventsListener cameraEventsListener) {
        ...
        this.mCameraManager2 = new CameraManager2(mCameraManager,ManualCameraX.getInstance(activity).getSettingsManager());
    ...
    private Size getCameraOutputSize(Size[] in, Size mPreviewSize) {
        ...
        List<Size> sizes = new ArrayList<>(Arrays.asList(in));
        int s = sizes.size() - 1;
        if (sizes.get(s).getWidth() * sizes.get(s).getHeight() <= ResolutionSolution.highRes || ManualCameraX.getSettings().QuadBayer) {
            target = sizes.get(s);
            if (ManualCameraX.getSettings().QuadBayer) {
    ...
    private void setUpCameraOutputs(int width, int height) {
        try {
            ...
            UpdateCameraCharacteristics(ManualCameraX.getSettings().mCameraID);
...
//            mCaptureSession.stopRepeating();
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        int rotation = ManualCameraX.getGravity().getRotation();//activity.getWindowManager().getDefaultDisplay().getRotation();
    ...
    public void restartCamera() {
        CameraFragment.mSelectedMode = ManualCameraX.getSettings().selectedMode;
        try {
            mCameraOpenCloseLock.acquire();
        ...
        StreamConfigurationMap map = this.mCameraCharacteristicsMap.get(ManualCameraX.getSettings().mCameraID).get(
        ...
        int max = 3;
        if (mTargetFormat == mPreviewTargetFormat) max = ManualCameraX.getSettings().frameCount + 3;
        //largest = target;
        ...
        mImageReaderRaw = ImageReader.newInstance(target.getWidth(), target.getHeight(),
                mTargetFormat, ManualCameraX.getSettings().frameCount + 3);
        ...
        try {
            ...
            this.mCameraManager.openCamera(ManualCameraX.getSettings().mCameraID, mStateCallback, mBackgroundHandler);
        } catch (CameraAccessException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
...
//                throw new RuntimeException("Time out waiting to lock camera opening.");
    ...
    public void UpdateCameraCharacteristics(String cameraId) {
        ...
        if (mTargetFormat == mPreviewTargetFormat)
            maxjpg = ManualCameraX.getSettings().frameCount + 3;
        ...
        mImageReaderRaw = ImageReader.newInstance(target.getWidth(), target.getHeight(), mTargetFormat, ManualCameraX.getSettings().frameCount + 3);
        mImageReaderRaw.setOnImageAvailableListener(mOnRawImageAvailableListener, mBackgroundHandler);
        // Find out if we need to swap dimension to get the preview size relative to sensor
        // coordinate.
        int displayRotation = ManualCameraX.getGravity().getRotation();
...
//            setUpMediaRecorder();
    ...
    private void captureStillPicture() {
        try {
            ...
            cameraRotation = ManualCameraX.getGravity().getCameraRotation(mSensorOrientation);
...
//                captureBuilder.set(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE, CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF);//Fix ois bugs for preview and burst
            ...
            captureBuilder.set(CaptureRequest.JPEG_ORIENTATION, ManualCameraX.getGravity().getCameraRotation(mSensorOrientation));
            ...
            Log.d(TAG, "HDRFact1:" + paramController.isManualMode() + " HDRFact2:" + ManualCameraX.getSettings().alignAlgorithm);
            ...
            IsoExpoSelector.useTripod = ManualCameraX.getGyro().getTripod();
            ...
            if (frameCount == -1) {
                ...
                ManualCameraX.getGyro().PrepareGyroBurst(times,BurstShakiness);
            }
...
//            final int[] burstcount = {0, 0, frameCount};
            ...
            this.CaptureCallback = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureStarted(@NonNull CameraCaptureSession session,
                    ...
                    if(maxFrameCount[0] != -1) ManualCameraX.getGyro().CaptureGyroBurst();
                }
                ...
                public void onCaptureSequenceCompleted(@NonNull CameraCaptureSession session,
                    ...
                    if(maxFrameCount[0] != -1) ManualCameraX.getGyro().CompleteGyroBurst();
                    //unlockFocus();
                    activity.runOnUiThread(() -> UpdateCameraCharacteristics(ManualCameraX.getSettings().mCameraID));
                    if(!isDualSession)
                        unlockFocus();
                    else
                        createCameraPreviewSession();
                    ManualCameraX.getGyro().CompleteSequence();
            ...
            if(isDualSession)
                ...
                switch (ManualCameraX.getSettings().selectedMode){
                    case NIGHT:
                    case UNLIMITED:
                        mCaptureSession.setRepeatingBurst(captures, CaptureCallback, null);
                        break;
...
//        setAFMode(mPreviewRequestBuilder, PreferenceKeys.getAfMode());
    ...
    public static class CameraProperties {
        ...
        private void logIt() {
            String lens = ManualCameraX.getSettings().mCameraID;
