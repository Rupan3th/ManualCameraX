...
public class Camera2ApiAutoFix {
    ...
    private void whitePoint() {
        CameraReflectionApi.set(SENSOR_NEUTRAL_COLOR_POINT, ManualCameraX.getCaptureController().mPreviewTemp);
    }

    private void CCM() {
        CameraReflectionApi.set(COLOR_CORRECTION_TRANSFORM, ManualCameraX.getCaptureController().mColorSpaceTransform);
    }
    ...
    public static void patchWL(CameraCharacteristics characteristics,
        ...
        if (patchWL != 0) {
            ...
            BlackLevel(characteristics, captureResult, ManualCameraX.getParameters().blackLevel,
                    (float) (patchWL) / ManualCameraX.getParameters().whiteLevel);
        }
    }
    ...
    public static void resetWL(CameraCharacteristics characteristics,
                               ...
                               int patchWL) {
        if (patchWL != 0) {
            WhiteLevel(captureResult, ManualCameraX.getParameters().whiteLevel);
            BlackLevel(characteristics, captureResult, ManualCameraX.getParameters().blackLevel,
                    1.f);
        }
    }
    ...
    public static void applyEnergySaving() {
        if (ManualCameraX.getSettings().energySaving) {
